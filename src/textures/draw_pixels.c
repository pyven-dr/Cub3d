...
static void	draw_wall_color(t_game_data *game_data, int i, int j, t_inter inter)
{
	if (inter.orientation == NORTH)
		...
		get_texture_color(&game_data->map_data.north, inter, \
						inter.pos_texture));
	if (inter.orientation == SOUTH)
		pixel_put(&game_data->mlx_data.img_data, j, i, \
		get_texture_color(&game_data->map_data.south, inter, \
						inter.pos_texture));
	if (inter.orientation == EAST)
		pixel_put(&game_data->mlx_data.img_data, j, i, get_texture_color(\
			&game_data->map_data.east, inter, inter.pos_texture));
	if (inter.orientation == WEST)
		pixel_put(&game_data->mlx_data.img_data, j, i, get_texture_color(\
			&game_data->map_data.west, inter, inter.pos_texture));
}
...
void	draw_column(t_game_data *game_data, int j, int wall_h, t_inter inter)
{
	...
	while (i < start_wall + wall_h && i < HEIGHT)
	{
